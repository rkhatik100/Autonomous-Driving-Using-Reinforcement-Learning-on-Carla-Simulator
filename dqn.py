...
for episode in range(episodes):
    ...
    for actor in env.actor_list:
            ...
            env.actor_list =[]  
    eps.append(epsilon)
    total_rewards_CARLA_DQN.append(success)
    losses.append(losses_)
    epsilon = epsilon*decay
    print(f"Episode {episode} and reward is {success} timestep: {timestep} time is: {time_taken}")
    if (episode %100 ==0):
        print(f"Episode: {episode} and reward: {np.mean(total_rewards_CARLA_DQN[-100:])}")#.format(episode,))
    if (episode % 4 ==0):
        target_net.load_state_dict(online_net.state_dict())
    if (episode % 4 ==0):
        torch.save(online_net.state_dict(), 'rkhatik_final_project_dqn_carla.pth')
torch.save(online_net.state_dict(), 'rkhatik_final_project_dqn_carla.pth')



plt.figure(figsize=(15, 10))
plt.plot(total_rewards_CARLA_DQN, 'r')
plt.xlabel('Episode', fontsize=28)
plt.ylabel('Reward Value', fontsize=28)
plt.title('Rewards Per Episode for CARLA DQN', fontsize=36)
plt.ylim(ymin=min(total_rewards_CARLA_DQN), ymax=max(total_rewards_CARLA_DQN)+1)
